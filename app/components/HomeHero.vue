...
const PATTERN = [
  ...
  { k: 0.95, src: '/images/hero/treatment-room', alt: 'The treatment room mid-appointment, Dr Deesha and a nurse at work' }, // next to centre — shortest
  { k: 1.34, src: '/images/hero/deesha-with-patient', alt: 'Dr Deesha with a smiling patient in the treatment chair at Smart Smiles' }, // central — tall
...
// eager set instead of at default image priority. Cards 3 and 4 are the tall
// centrals — the largest cards on desktop. Every later copy repeats these
// same six URLs, so boosting the first copy covers them all.
...
const MOBILE_ANCHOR_CARD = START_COPIES * SLOTS_PER_COPY + Math.floor(SLOTS_PER_COPY / 2) + 1
...
const LCP_IMAGE = PATTERN[Math.floor(SLOTS_PER_COPY / 2)]!.src
useHead({
  link: [
    {
      rel: 'preload',
