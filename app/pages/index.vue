...
const sizes = ['a', 'b', 'c', 'd', 'b', 'd', 'a', 'c', 'c', 'a', 'd', 'b', 'd', 'c', 'b', 'a', 'a', 'd']
...
useHead({
  link: [
    ...
    { rel: 'stylesheet', href: 'https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,300&family=Bodoni+Moda:opsz,wght@6..96,400;6..96,700&display=swap' },
  ],
})
...
onMounted(() => {
  ...
  const byPosition = [...cardEls].sort((a, b) => {
    const ra = a.getBoundingClientRect()
    const rb = b.getBoundingClientRect()
    return ra.top !== rb.top ? ra.top - rb.top : ra.left - rb.left
  })

  const firstNine = gsap.utils.shuffle(byPosition.slice(0, 9))
  const rest = byPosition.slice(9)
